...
class WPMenuEditor extends MenuEd_ShadowPluginFramework {
   ...
   */
	...
	function enqueue_scripts(){
		//jQuery JSON plugin
		wp_register_auto_versioned_script('jquery-json', MEURL . 'js/jquery.json.js', array('jquery'));
		//jQuery sort plugin
		wp_register_auto_versioned_script('jquery-sort', MEURL . 'js/jquery.sort.js', array('jquery'));
		//qTip2 - jQuery tooltip plugin
		wp_register_auto_versioned_script('jquery-qtip', MEURL . 'js/jquery.qtip.min.js', array('jquery'));
		//jQuery Form plugin. This is a more recent version than the one included with WP.
		wp_register_auto_versioned_script('ame-jquery-form', MEURL . 'js/jquery.form.js', array('jquery'));
		//jQuery cookie plugin
		wp_register_auto_versioned_script('jquery-cookie', MEURL . 'js/jquery.cookie.js', array('jquery'));

		//Editor's scripts
		wp_register_auto_versioned_script(
			'menu-editor',
			MEURL . 'js/menu-editor.js',
			array(
	...
	function enqueue_styles(){
		wp_enqueue_auto_versioned_style('jquery-qtip-syle', MEURL . 'css/jquery.qtip.min.css', array());

		wp_register_auto_versioned_style('menu-editor-base-style', MEURL . 'css/menu-editor.css');
		wp_register_auto_versioned_style(
			'menu-editor-colours-classic',
			MEURL . 'css/style-classic.css',
			array('menu-editor-base-style')
		);
		wp_register_auto_versioned_style(
			'menu-editor-colours-wp-grey',
			MEURL . 'css/style-wp-grey.css',
			array('menu-editor-base-style')
		);
		...
		wp_register_auto_versioned_style('menu-editor-screen-meta', MEURL . 'css/screen-meta.css');
		wp_register_auto_versioned_style('menu-editor-screen-meta-old', MEURL . 'css/screen-meta-old-wp.css');
   ...
   */
	...
	private function handle_form_submission($post, $action = '') {
		if ( $action == 'save_menu' ) {
			...
			if ( isset($post['data']) ){
				...
				try {
					$menu = ameMenu::load_json(stripslashes($post['data']), true);
	...
	public function enqueue_menu_fix_script() {
		wp_enqueue_auto_versioned_script(
			'ame-menu-fix',
			MEURL . 'js/menu-highlight-fix.js',
			array('jquery'),
			true
		);
	}
